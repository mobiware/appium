...
androidCommon.toggleSetting = function(setting, ocb) {
  var doDown = function(cb) {
    setTimeout(function() {
      this.adb.keyevent(20, cb);
    }.bind(this), 2000);
  }.bind(this);
  ...
  async.series([
    ...
    function(cb) {
      if(setting === "AIRPLANE_MODE_SETTINGS") {
        this.adb.getApiLevel(function(err, api) {
          if(api > 15) {
            doDown(cb);
          } else {
    ...
    , function(cb) {
      this.wrapActionAndHandleADBDisconnect(function(cb) {
        setTimeout(function() {
          this.adb.keyevent(23, cb);
        }.bind(this), 3000);
...
androidCommon.toggleData = function(ocb) {
  this.toggleSetting('DATA_ROAMING_SETTINGS', ocb);
};

androidCommon.toggleFlightMode = function(ocb) {
  this.toggleSetting('AIRPLANE_MODE_SETTINGS', ocb);
};

androidCommon.toggleWiFi = function(ocb) {
  this.toggleSetting('SETTINGS', ocb);
};

androidCommon.toggleGPS = function(ocb) {
  this.toggleSetting('LOCATION_SOURCE_SETTINGS', ocb);
};
