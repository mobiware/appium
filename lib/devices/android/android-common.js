...
androidCommon.restartAdb = function(cb) {
  async.series([
    this.adb.restartAdb.bind(this.adb)
    , this.adb.waitForDevice.bind(this.adb)
  ], cb);
};
...
androidCommon.toggleSetting = function(setting, preKeySeq, ocb) {
  ...
  var back = function(cb) {
    this.adb.back(function(rr) {
      if(rr) {
        cb(rr);
      } else {
        this.adb.waitForNotActivity(settingsPackage, settingsActivity, 5000, cb);
      }
    }.bind(this));
  }.bind(this);

  if(preKeySeq === null) preKeySeq = [19, 19, 20];
  ...
  var settingsPackage, settingsActivity;

  sequence.push(
    function(cb) {
      this.adb.getFocusedPackageAndActivity(function(err, foundPackage,
                                                     foundActivity) {
        settingsPackage = foundPackage;
        settingsActivity = foundActivity;
        cb(err);
      }.bind(this));
    }.bind(this)
    , function(cb) {
    ...
    , function(cb) {
      this.adb.waitForNotActivity(settingsPackage, settingsActivity, 5000, function(err) {
        if(err) {
          cb(null);
        } else {
          async.series([
            ...
            , doKey(23)
            , function(cb) {
              this.adb.waitForActivity(settingsPackage, settingsActivity, 5000, cb);
            }.bind(this)
          ], function(err) {
            cb(err);
          }.bind(this));
  ...
  setTimeout(function() {
    ...
    this.adb.shell("am start -n "+ appName, function(err, stdout) {
      ...
      if (stdout.indexOf("Error:") !== -1) {
        return cb(new Error("Error switching app back to foreground"));
      }
      cb(null, stdout.trim());
    });
  }.bind(this),secs *1000);
